...
class PDController(Node):
    ...
    def odom_cb(self, msg: Odometry):
        ...
        self.get_logger().info(f"[ODOM_CB] yaw actual: {math.degrees(self.current_yaw):.2f}")
    ...
    def control_loop(self):
        if not self.odom_ready:
            self.get_logger().warn("[PD] Esperando odometría...")
            return
    ...
    f"[CTRL] error={math.degrees(error):.2f}° angular={angular:.3f}"
)
